...
function RowInformation(props) {
  const [beerDropdown, setBeerDropdown] = useState({
    isActive: false,
    data: {},
  });
      ...
      >
        <div className="menuRowItem">
          ...
          <h2 className="priceHeader bgb">
            {" "}
            <span className="cupSize">{props.data.price}</span>/
            <span className="cupSize">{props.data.price + 6}</span>
          </h2>
        </div>
      </div>
      <BeerDropdown
        itemId={props.data.id}
        isActive={beerDropdown.isActive}
        data={props.data}
      />
    </div>
  );
